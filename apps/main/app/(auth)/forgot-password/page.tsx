...
const ForgotPasswordContent: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (error) {
        toast.error(error.message || t.forgotPassword.errors.failed);
      } else {
        ...
        <div className="w-full max-w-[480px]">
          <div className="bg-white rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.06)] border border-slate-100 p-10 md:p-14 w-full">
            <div className="text-center mb-10 space-y-4">
              ...
              <h2 className="text-3xl font-black text-slate-900 leading-tight">{t.forgotPassword.title}</h2>
              <p className="text-slate-400 text-sm font-medium italic">{t.forgotPassword.subtitle}</p>
            </div>

            {!isSent ? (
              <form onSubmit={handleSubmit} className="space-y-8">
                <div className="space-y-2">
                  <label htmlFor="email" className="block text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">{t.forgotPassword.emailLabel}</label>
                  <input
                    type="email" id="email"
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex justify-center pt-2">
                    <Link href="/sign-in" className="group text-[10px] font-black text-slate-400 hover:text-orange transition-all flex items-center gap-2 uppercase tracking-[0.25em]">
              ...
              <div className="text-center py-8 space-y-8">
                ...
                <div className="space-y-4">
                  <h3 className="text-2xl font-black text-slate-900">{t.forgotPassword.checkEmail}</h3>
                  <p className="text-slate-400 font-medium italic text-sm leading-relaxed">
                    {t.forgotPassword.sentMessage.split("{email}")[0]}
                    <span className="text-slate-900 font-black underline decoration-orange/30 underline-offset-4">{email}</span>
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <Link href="/sign-in" className="inline-flex items-center justify-center w-full py-5 rounded-2xl bg-slate-900 text-white text-[10px] font-black shadow-xl hover:bg-slate-800 hover:-translate-y-1 transition-all uppercase tracking-[0.25em]">
                    {t.forgotPassword.backToSignIn}
                  </Link>
                </div>
              </div>
