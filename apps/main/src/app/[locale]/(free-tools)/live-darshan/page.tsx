...
const LiveDarshanPage = () => {
  ...
  const sites = messages.Livedarshan.sites as typeof Livedarshan.sites;
  ...
  useEffect(() => {
    const fetchDarshanSites = async () => {
      try {
        ...
        const localizedSites: DarshanSite[] = sites.map((site, index) => ({
          id: String(site.id),
      ...
      <section className="relative py-24 md:py-32 px-4 overflow-hidden bg-brown text-white">
        ...
        <div className="container relative z-10 text-center">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-display font-black mb-8 tracking-tight text-white leading-tight">
            {t("heroTitle")}{" "}
            <span className="text-orange drop-shadow-sm">
              {t("heroHighlight")}
            </span>
          </h1>
            ...
            {filteredDarshanSites.length > 0 ? (
                ...
                >
                  <div className="relative h-64 overflow-hidden">
                    ...
                    <div className="absolute top-4 right-4 z-20">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 ${site.status === "Live Now" || site.status === "अभी लाइव" ? "bg-red-500 text-white" : "bg-gray-500 text-white"}`}
                      >
              ...
              <div className="col-span-1 md:col-span-2 lg:col-span-3 text-center py-16">
                ...
                <p className="text-gray-500 font-body">
                  {t("noTemplesDesc").replace("{query}", searchQuery)}
                </p>
              </div>
            )}
          </div>
