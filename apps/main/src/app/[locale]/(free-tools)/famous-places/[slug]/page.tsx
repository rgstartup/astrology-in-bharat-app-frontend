...
const PlaceDetailPage = () => {
  ...
  const [distanceInfo, setDistanceInfo] = useState<{
    ...
    status: "idle" | "requesting" | "granted" | "denied" | "error" | "no-dest";
  }>({
    distance: null,
    status: "idle",
  });
  ...
  const handleGetLocation = () => {
    ...
    const destLat =
      (place as any).gps_coordinates?.latitude || (place as any).latitude;
    const destLon =
      (place as any).gps_coordinates?.longitude || (place as any).longitude;

    const requestUserLocationAndCalculate = (
      targetLat: number,
      targetLon: number,
    ) => {
    ...
    if (destLat && destLon) {
      ...
      fetchWithQuery(searchQueries)
        .then((data) => {
          if (data) {
            requestUserLocationAndCalculate(
              parseFloat(data.lat),
              parseFloat(data.lon),
            );
          } else {
  ...
  const getDynamicHighlights = (title: string = "") => {
    ...
    if (
      lowerTitle.includes("gurudwara") ||
      lowerTitle.includes("sahib") ||
      lowerTitle.includes("golden temple")
    ) {
      return [
        "Langar Seva (Free Food)",
        "24x7 Kirtan & Prayers",
        "Sarovar (Holy Pool)",
        "Community Service",
      ...
      lowerTitle.includes("balaji")
    ) {
      return [
        "Sacred Atmosphere",
        "Tuesday/Saturday Special Puja",
        "Divine Energy",
        "Ancient Idols",
      ...
      lowerTitle.includes("laxmi") ||
      lowerTitle.includes("narayan") ||
  ...
  const getDynamicAstrologyBenefits = (title: string = "") => {
    ...
    if (lowerTitle.includes("shiv") || lowerTitle.includes("mahadev")) {
      return [
        ...
        "Strengthens the Moon in your horoscope",
      ...
      return [
        ...
        "Protects from evil eyes (Buri Nazar)",
        "Strengthens Mars in kundli",
      ...
      return [
        ...
        "Improves Venus and Jupiter alignments",
        "Harmonizes married life",
        "Success in career",
      ];
    } else if (
      lowerTitle.includes("durga") ||
      lowerTitle.includes("mata") ||
      lowerTitle.includes("devi")
    ) {
      return [
        "Empowers with strength and protection",
        "Overcomes obstacles in life",
  ...
  const handleShare = (platform: string) => {
    ...
    const text = encodeURIComponent(
      `Check out ${place.title} on Astrology In Bharat!`,
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white p-6">
        <div className="text-center p-6 bg-white rounded-lg border border-slate-200 max-w-sm">
          <h2 className="text-base font-bold text-brown mb-1">
            {t("detail.notFound")}
          </h2>
          <Link href="/famous-places" className="text-xs font-bold text-orange">
    ...
    <div className="min-h-screen bg-[#FAF8F5] pb-12">
      <div className="max-w-[1300px] mx-auto px-4 md:px-8 pt-6">
        ...
        <div className="flex items-center gap-2 text-sm font-semibold text-[#F26500] mb-6">
          ...
          <Link href="/famous-places" className="hover:underline">
            Famous Temples
          </Link>
          <span className="text-gray-400">›</span>
        ...
        <div className="bg-white rounded-[24px] p-6 shadow-sm border border-[#E8D5C0] mb-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col">
              <h1
                className="text-3xl md:text-[42px] font-serif font-black text-[#3D1A0B] mb-2 leading-tight truncate"
                title={place.title}
              >
                {place.title}
              </h1>
              ...
              <div className="flex items-center gap-3 mb-6">
                <div className="text-[15px] font-bold text-gray-800">
                  {place.rating || "4.9"}{" "}
                  <i className="fa-solid fa-star text-[#F26500]"></i>
                  <span className="text-gray-500 ml-1">
                    {/* ({place.reviews || "24K"} Reviews) */}
                    {"24K Reviews"}
                  </span>
                </div>
                ...
                {distanceInfo.status === "idle" && (
                  <button
                    ...
                    className="text-[13px] font-bold text-[#F26500] hover:underline flex items-center gap-1"
                  >
                    <i className="fa-solid fa-location-crosshairs"></i> Get
                    Distance
                  </button>
                )}
                ...
                {distanceInfo.status === "granted" && (
                  <div className="text-[14px] font-bold text-green-600 flex items-center gap-1">
                    ...
                    {distanceInfo.distance} km from you
                  </div>
                )}
                {distanceInfo.status === "denied" && (
                  <div
                    className="text-[13px] font-bold text-red-500"
                    title="Please enable location permissions in your browser"
                  >
                    Location Denied
                  </div>
                )}
                {(distanceInfo.status === "no-dest" ||
                  distanceInfo.status === "error") && (
              ...
              <p className="text-gray-600 leading-relaxed mb-8 text-[15px]">
                {place.description ||
                  "" ||
                  `Sri Harmandir Sahib, also known as the ${place.title}, is the most revered spiritual center. It welcomes people from all walks of life.`}
              </p>

              {/* 4 Features */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 border-y border-dashed border-[#E8D5C0] py-6">
                <div className="flex flex-col items-center justify-center text-center gap-2">
                  <div className="text-[#F26500]">
                    <i className="fa-regular fa-clock text-xl"></i>
                  </div>
                  <div className="text-[11px] font-bold text-gray-500">
                    Open
                  </div>
                  <div className="text-[13px] font-bold text-[#3D1A0B]">
                    24 Hours
                  </div>
                </div>
                <div className="flex flex-col items-center justify-center text-center gap-2">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-8">
                ...
                <button
                  ...
                  className="w-full sm:flex-1 bg-white border-[2px] border-[#D95F18] text-[#D95F18] hover:bg-orange-50 font-bold py-3.5 px-4 sm:px-6 text-[15px] sm:text-base rounded-xl transition-colors flex items-center justify-center gap-2"
                >
        ...
        <div className="flex items-center gap-3 overflow-x-auto pb-4 scrollbar-hide mb-6">
          {[
            ...
            return (
              ...
              >
                <i className={`fa-solid ${tab.icon} text-[#F26500]`}></i>{" "}
                {tab.label}
              </button>
            );
          })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <div
              id="about"
              className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E8D5C0]"
            >
              <h3 className="text-[20px] font-bold text-[#3D1A0B] mb-4 flex items-center gap-2">
                <i className="fa-brands fa-pagelines text-[#F26500]"></i> About{" "}
                {place.title}
              </h3>
              <div className="text-gray-600 text-[15px] leading-relaxed space-y-4 mb-6">
                <p>
                  {place.description ||
                    `${place.title} is a highly revered spiritual destination located in ${place.address || "India"}. It is renowned for its stunning architecture, serene ambiance, and divine presence.`}
                </p>
                <p>
                  The temple welcomes millions of devotees and tourists every
                  year, symbolizing purity and equality. Visitors can experience
                  peace, participate in daily rituals, and seek blessings.
                </p>
              </div>
              <div className="flex flex-wrap justify-center sm:justify-start gap-3 mt-8">
                {[
                  ...
                  <div
                    key={i}
                    className="bg-[#FFF8F0] text-[#3D1A0B] font-bold text-[13px] px-4 py-2.5 rounded-lg flex items-center gap-2 border border-[#F26500]/10"
                  >
                    <i className="fa-solid fa-dharmachakra text-[#F26500]"></i>{" "}
                    {pill}
                  </div>
                ))}
              </div>
            ...
            >
              ...
              <p className="text-gray-600 text-[15px] leading-relaxed">
                The foundation of {place.title} is deeply rooted in history and
                tradition. It has witnessed various historical events but
                continues to stand as a powerful symbol of faith, humanity, and
            ...
            <div
              id="astrology"
              className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E8D5C0] relative overflow-hidden"
            >
              <h3 className="text-[20px] font-bold text-[#3D1A0B] mb-4 flex items-center gap-2 relative z-10">
                <i className="fa-regular fa-sun text-[#F26500]"></i> Astrology
                Benefits
              </h3>
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-between">
                <div className="flex-1 space-y-4">
                  ...
                  <ul className="space-y-4 relative z-10">
                    {getDynamicAstrologyBenefits(place.title).map(
                      (benefit, i) => (
                        <li
                          key={i}
                          className="flex items-center gap-3 text-gray-600 text-[14px]"
                        >
            ...
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                <h3 className="text-[20px] font-bold text-[#3D1A0B] flex items-center gap-2">
                  <i className="fa-solid fa-comment-dots text-[#F26500]"></i>{" "}
                  Reviews & Ratings
                </h3>
                <a
              ...
              <div className="flex flex-col sm:flex-row items-center gap-8 sm:gap-10">
                <div className="text-center min-w-[120px]">
                  <div className="text-5xl font-black text-[#3D1A0B]">
                    {place.rating || "4.8"}
                  </div>
                  <div className="flex text-[#F26500] text-sm justify-center my-2 gap-1">
                    {[...Array(5)].map((_, i) => (
                      <i
                        key={i}
                        className={`fa-solid fa-star ${i < Math.floor(Number(place.rating) || 4) ? "" : "text-gray-300"}`}
                      ></i>
                    ))}
                  </div>
                  <div className="text-sm text-gray-500 font-medium">
                    {place.reviews || "1,200+"} reviews
                  </div>
                </div>
                    ...
                    <div key={star} className="flex items-center gap-3 text-sm">
                      ...
                      <div className="flex-1 h-2.5 bg-[#2A2A2A] rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${star === 5 ? 75 : star === 4 ? 15 : star === 3 ? 5 : star === 2 ? 3 : 2}%`,
                          }}
                        ></div>
                      </div>
                    </div>
          ...
          <div className="lg:col-span-1 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E8D5C0]">
              <h3 className="text-[20px] font-bold text-[#3D1A0B] mb-4 flex items-center gap-2">
                <i className="fa-solid fa-map-location-dot text-[#F26500]"></i>{" "}
                Location
              </h3>
              <a
                href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(place.title + " " + (place.address || ""))}`}
              ...
              >
                ...
                <p className="text-sm font-bold text-[#3D1A0B] group-hover:text-[#F26500] transition-colors m-0">
              ...
              <div className="flex items-start gap-3 text-gray-600 text-[14px] font-medium mb-6">
                ...
                <p className="leading-relaxed">
                  {place.title}, {place.address || "India"}
                </p>
              </div>
              <a
                href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(place.title + " " + (place.address || ""))}`}
            ...
            <div className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E8D5C0]">
              <h3 className="text-[20px] font-bold text-[#3D1A0B] mb-5 flex items-center gap-2">
                <i className="fa-solid fa-building-columns text-[#F26500]"></i>{" "}
                Temple Highlights
              </h3>
              <ul className="space-y-4">
                {getDynamicHighlights(place.title).map((highlight, i) => (
                  <li
                    key={i}
                    className="flex items-center gap-3 text-gray-600 text-[14px] font-medium"
                  >
                    <i className="fa-regular fa-circle-dot text-[#F26500]"></i>
                    <span>{highlight}</span>
                  </li>
            ...
            <div className="bg-white rounded-[20px] p-6 shadow-sm border border-[#E8D5C0]">
              <h3 className="text-[20px] font-bold text-[#3D1A0B] mb-5 flex items-center gap-2">
                <i className="fa-solid fa-share-nodes text-[#F26500]"></i> Share
                this Temple
              </h3>
              <div className="flex items-center justify-between gap-2">
                <button
                  onClick={() => handleShare("facebook")}
                  className="w-12 h-12 rounded-full border border-gray-200 shadow-sm flex items-center justify-center hover:shadow-md transition-shadow bg-white text-[#1877F2]"
                >
                  <i className="fa-brands fa-facebook-f text-xl"></i>
                </button>
                <button
                  onClick={() => handleShare("whatsapp")}
                  className="w-12 h-12 rounded-full border border-gray-200 shadow-sm flex items-center justify-center hover:shadow-md transition-shadow bg-white text-[#25D366]"
                >
                  <i className="fa-brands fa-whatsapp text-xl"></i>
                </button>
                <button
                  onClick={() => handleShare("twitter")}
                  className="w-12 h-12 rounded-full border border-gray-200 shadow-sm flex items-center justify-center hover:shadow-md transition-shadow bg-white text-[#1DA1F2]"
                >
                  <i className="fa-brands fa-twitter text-xl"></i>
                </button>
                <button
                  onClick={() => handleShare("email")}
                  className="w-12 h-12 rounded-full border border-gray-200 shadow-sm flex items-center justify-center hover:shadow-md transition-shadow bg-white text-[#EA4335]"
                >
