...
const HoroscopeSidebar = () => {
  ...
  useEffect(() => {
    ...
    const fetchAstroData = async () => {
      ...
      const [panchangRes] = await api.get<any>(
        `${API_ROUTES.ASTROLOGY.PANCHANG}?datetime=${datetime}&lat=${lat}&lon=${lon}&lang=${lang}`,
      );
      if (panchangRes?.data) {
        setPanchang(panchangRes.data);
      }

      const [planetRes] = await api.get<any>(
        `${API_ROUTES.ASTROLOGY.PLANETARY_POSITIONS}?datetime=${datetime}&lat=${lat}&lon=${lon}&lang=${lang}`,
      );
      if (planetRes?.data) {
  ...
  const displayPlanets =
    planets && planets.length > 0
      ? planets.map((p) => ({
          ...
          sign:
            t.zodiacNames[p.zodiac?.name as keyof typeof t.zodiacNames] ||
            p.zodiac?.name ||
            p.position ||
            "Unknown",
        }))
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="bg-[#FAF8F5] rounded-xl p-6 border border-[#E8D5C0]">
        <h3 className="text-[18px] font-bold text-[#3D1A0B] mb-5 flex items-center gap-2">
          <i className="fa-regular fa-sun text-[#F26500]"></i>{" "}
          {t.sidebar.overview}
        </h3>
        <div className="flex flex-col gap-4">
          <div className="flex gap-3">
            ...
            <div>
              <p className="text-[13px] font-bold text-[#3D1A0B]">
                {t.sidebar.date}
              </p>
              <p className="text-[13px] text-gray-800">
                {formattedDate || "18 May 2024, Saturday"}
              </p>
            </div>
          </div>
          <div className="flex gap-3">
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-[13px] text-gray-800">
                {panchang?.nakshatra?.details?.[0]?.name ||
          ...
          <div className="flex items-center justify-between mt-2 pt-4 border-t border-[#E8D5C0]">
            <div className="flex flex-col items-center">
              ...
              <p className="text-[12px] font-bold text-[#3D1A0B]">
                {t.sidebar.sunrise}
              </p>
              <p className="text-[11px] text-gray-700">
                {formatTime(panchang?.sunrise) || "05:33 AM"}
              </p>
            </div>
            <div className="flex flex-col items-center">
              ...
              <p className="text-[11px] text-gray-700">
                {formatTime(panchang?.sunset) || "07:05 PM"}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-[#2D1205] rounded-xl p-6 relative overflow-hidden border border-[#3D1A0B]">
        <div className="relative z-10">
          <h3 className="text-[15px] sm:text-[18px] font-bold text-[#F26500] mb-2 whitespace-nowrap">
            {t.sidebar.personalizedTitle}
          </h3>
          ...
          <Link
            href="/our-experts"
            className="bg-[#F26500] hover:bg-orange-600 transition-colors text-white text-[14px] font-bold px-4 py-2 rounded-lg flex items-center justify-center gap-2 w-fit"
          >
