...
const ResultPanel = ({ result }: { result: any }) => {
  const t = useTranslations("LoveCalculator");
  ...
  return (
    <div className="bg-white border border-[#F0E0D0] rounded-3xl p-6 md:p-8 flex flex-col gap-6 h-full">
      ...
      {/* Detailed Report btn */}
      <button className="w-full border border-[#F26500] text-[#F26500] rounded-2xl py-3 font-bold flex items-center justify-center gap-2 hover:bg-[#FFF5EE] transition-colors cursor-pointer">
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err || !data) {
        ...
        toast.error(getErrorMessage(data) || t("form.errors.generic"));
      }
    } finally {
      setLoading(false);
    }
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-4 py-4">
        <p className="text-sm text-[#888]">
          <span className="text-[#888]">{t("breadcrumb.home")}</span>
          <span className="mx-2">›</span>
          <span className="text-[#F26500] font-semibold">{t("breadcrumb.calculators")}</span>
          <span className="mx-2">›</span>
        ...
        >
          ...
          >
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#F26500] hover:bg-[#D95A00] text-white font-bold rounded-2xl py-3.5 flex items-center justify-center gap-2 transition-colors disabled:opacity-70 mt-1 cursor-pointer"
            >
              {loading ? (
