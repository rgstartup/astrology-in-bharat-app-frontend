...
function buildConicGradient(gunas: any[]) {
  ...
  const total = gunas.reduce(
    (acc: number, g: any) => acc + (g.maximum_points || 1),
    0,
  );
  let angle = 0;
...
const ResultComponent = ({
  ...
  const getGunaRating = (obtained: number, max: number) => {
    if (obtained === max)
      return {
        text: t("results.excellent"),
        color: "text-green-600",
        icon: <FaCheckCircle className="text-green-500 text-[10px]" />,
      };
    if (obtained >= max / 2)
      return {
        text: t("results.good"),
        color: "text-orange-500",
        icon: <FaCheckCircle className="text-orange-400 text-[10px]" />,
      };
    return {
      text: t("results.average"),
      color: "text-amber-500",
      icon: <FaExclamationTriangle className="text-amber-400 text-[10px]" />,
    };
  };
        ...
        <div className="bg-white rounded-[2rem] border border-gray-100 shadow-[0_8px_40px_rgba(0,0,0,0.05)] overflow-hidden p-6 md:p-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-orange-200 text-orange-600 text-[12px] font-semibold hover:bg-orange-50 transition-colors cursor-pointer">
                <FiFileText className="text-sm" /> {t("results.viewReport")}
              </button>
              <button className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-orange-200 text-orange-600 text-[12px] font-semibold hover:bg-orange-50 transition-colors cursor-pointer">
                <FiShare2 className="text-sm" /> {t("results.shareResult")}
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mb-8">
            ...
            <div className="lg:col-span-4 bg-[#fff9f4] border border-orange-100 rounded-2xl p-6 flex flex-col">
              <p className="text-[13px] font-bold text-slate-800 text-center mb-5">
                {t("results.overallCompatibility")}
              </p>

              {/* Score */}
              <div className="flex items-baseline justify-center gap-1 mb-3">
                <span
                  className={`text-[56px] leading-none font-black ${isExcellent ? "text-[#f95700]" : "text-slate-700"}`}
                >
                  {totalScore}
                </span>
                <span className="text-[22px] font-bold text-[#f95700]">
                  / {maxScore}
                </span>
              </div>
              ...
              <div className="mt-5 bg-[#f0f9f3] border border-green-100 rounded-xl p-3.5 flex gap-2.5">
                ...
                <p className="text-[11px] text-green-800 font-medium leading-relaxed">
                  {isExcellent
                    ? t("results.excellentDesc")
                    : t("results.moderateDesc")}
                </p>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 bg-[#fff9f4] border border-orange-100 rounded-2xl p-6">
              <p className="text-[13px] font-bold text-slate-800 mb-5">
                {t("results.gunaMilanScore")}
              </p>
              ...
              <div className="flex items-center gap-5">
                ...
                >
                  <div className="w-full h-full bg-[#fff9f4] rounded-full flex flex-col items-center justify-center">
                    <span className="text-[26px] font-black text-[#f95700] leading-none">
                      {totalScore}
                    </span>
                ...
                <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                  {gunas.slice(0, 8).map((item: any, idx: number) => {
                    const gunaName =
                      GUNA_NAME_BY_ID[item.id] ??
                      safeStr(item.name, `Guna ${idx + 1}`);
                    return (
                      <div
                        key={idx}
                      ...
                      >
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span
                            ...
                            style={{
                              background:
                                GUNA_COLORS[idx % GUNA_COLORS.length],
                            }}
                          />
            ...
            <div className="lg:col-span-4 flex flex-col gap-3">
              {/* Manglik */}
              <div
                className={`flex-1 rounded-2xl border p-4 flex gap-3 items-center ${hasManglik ? "bg-red-50 border-red-100" : "bg-[#fffcf8] border-orange-100"}`}
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${hasManglik ? "bg-red-100 text-red-500" : "bg-orange-100 text-orange-500"}`}
                >
                  <FaFire className="text-base" />
                </div>
                <div>
                  <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide mb-0.5">
                    {t("results.manglikStatus")}
                  </p>
                  <h4
                    className={`text-[13px] font-bold ${hasManglik ? "text-red-600" : "text-slate-800"}`}
                  >
                    {hasManglik
                      ? t("results.manglikPresent")
                      : t("results.bothNonManglik")}
                  </h4>
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    {t("results.noManglikDosha")}
                  </p>
                </div>
              </div>
              ...
              <div className="flex-1 rounded-2xl border border-gray-100 bg-white p-4 flex gap-3 items-center">
                ...
                <div>
                  <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide mb-0.5">
                    {t("results.bhakootDosha")}
                  </p>
                  <h4 className="text-[13px] font-bold text-green-600">
                    {t("results.notPresent")}
                  </h4>
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    {t("results.noBhakootDosha")}
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {gunas.map((item: any, idx: number) => {
                const gunaName =
                  GUNA_NAME_BY_ID[item.id] ??
                  safeStr(item.name, `Guna ${idx + 1}`);
                const rating = getGunaRating(
                  item.obtained_points,
                  item.maximum_points,
                );
                const iconStyle =
                  ICON_STYLE[gunaName] ?? "bg-orange-50 text-orange-400";
                return (
                  <div
                    key={idx}
                    className="bg-white border border-orange-100/60 rounded-2xl p-4 flex items-center gap-3 hover:shadow-sm transition-shadow"
                  >
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${iconStyle}`}
                    >
                      <i
                        className={`fa-solid ${GUNA_ICONS[gunaName] || "fa-star"} text-base`}
                      />
                    </div>
