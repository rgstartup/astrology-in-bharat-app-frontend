      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex space-x-8 overflow-x-auto pb-4 scrollbar-hide snap-x items-center px-4">
          {ZodiacSignsData.map((sign) => (
            ...
            >
              <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-3 transition-transform duration-500 ${selectedSign.id === sign.id ? "scale-110" : "group-hover:scale-110 animate-slow-float"}`}
              >
                <Image
                  src={sign.image}
                  alt={sign.title}
              ...
              <span
                className={`text-[9px] font-black uppercase tracking-[0.2em] ${selectedSign.id === sign.id ? "text-yellow-700" : "text-gray-500"}`}
              >
                {sign.title}
              </span>
            </button>
