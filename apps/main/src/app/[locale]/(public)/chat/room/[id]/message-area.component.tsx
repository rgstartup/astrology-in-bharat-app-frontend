...
import { FileText, X, AlertCircle, User } from "lucide-react";
    ...
    >
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="flex justify-center mb-10">
          <div
            className={`px-6 py-2 rounded-full text-[11px] font-bold uppercase tracking-[0.2em] transition-all border ${sessionSummary?.status === "terminated" ? "bg-red-500/10 text-red-500 border-red-500/20" : isDarkMode ? "bg-[#1A1A1A]/5 opacity-30 border-white/5" : "bg-[#fd6410]/10 text-[#fd6410] border-[#fd6410]/20"}`}
          >
        ...
        {messages.map((msg: any, index: number) => {
          ...
          const isOnlyAttachment =
            hasAttachment &&
            (!msg.content ||
              msg.content.trim() === "" ||
              msg.content.trim().toLowerCase() === "sent an image");

          return (
            <div
              key={msg.id}
              className={`flex gap-3 md:gap-4 ${isUser ? "flex-row-reverse" : "flex-row"} items-start ${isAdmin ? "justify-center w-full" : ""}`}
            >
              {!isAdmin && (
                <div className="flex-shrink-0 mt-1">
                  <div
                    className={`w-8 h-8 rounded-full border-2 ${isUser ? "border-[#fd6410]/30" : "border-black/5"} overflow-hidden shadow-sm flex items-center justify-center bg-[#2A2A2A]`}
                  >
                    {isUser ? (
              ...
              <div
                className={`flex flex-col gap-1.5 max-w-[75%] md:max-w-[70%] ${isAdmin ? "items-center w-full" : ""}`}
              >
                <div
                  className={`${
                    msg.content.startsWith("[INTRO_CARD]")
                      ? ""
                      : `${isOnlyAttachment ? "p-1" : "px-4 py-3"} rounded-2xl shadow-sm text-sm leading-relaxed ${
                          isAdmin
                ...
                >
                  ...
                  {msg.content.startsWith("[INTRO_CARD]") ? (
                    <div className="bg-gradient-to-br from-[#FFD700] via-[#FFEA00] to-[#FFD700] p-0.5 rounded-2xl shadow-lg border border-black/10 w-full min-w-[240px]">
                      <div className="bg-yellow-400/20 backdrop-blur-sm rounded-xl p-4 border border-white/20 relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-2 opacity-10 pointer-events-none">
                          <User className="w-16 h-16 text-yellow-900" />
                        </div>
                        <div className="flex items-center gap-2 mb-4 border-b border-yellow-900/10 pb-3 relative z-10">
                          ...
                          <div>
                            <h3 className="text-yellow-900 font-black text-xs uppercase tracking-widest">
                              Birth Details
                            </h3>
                          </div>
                        </div>
                          ...
                          {(() => {
                            try {
                              const data = JSON.parse(
                                msg.content.replace("[INTRO_CARD]", ""),
                              );
                              ...
                              return (
                                <>
                                  <div className="space-y-1">
                                    <span className="text-[9px] font-black text-yellow-900/50 uppercase tracking-widest">
                                      Name
                                    </span>
                                    <p className="text-sm font-black text-yellow-900 truncate">
                                      {data.name || "N/A"}
                                    </p>
                                  </div>
                                  <div className="space-y-1">
                                    ...
                                    <p className="text-sm font-black text-yellow-900 capitalize">
                                      {data.gender || "N/A"}
                                    </p>
                                  </div>
                                  <div className="space-y-1">
                                    ...
                                    <p className="text-sm font-black text-yellow-900">
                                      {formatDate(data.dob || data.dateOfBirth)}
                                    </p>
                                  </div>
                                  <div className="space-y-1">
                                    <span className="text-[9px] font-black text-yellow-900/50 uppercase tracking-widest">
                                  ...
                                  <div className="space-y-1 col-span-2">
                                    ...
                                    <p className="text-sm font-black text-yellow-900 truncate">
                                      {data.pob || data.placeOfBirth || "N/A"}
                                    </p>
                                  </div>
                                </>
                              );
                    ...
                    <>
                      {hasAttachment && (
                        <div
                          className={`max-w-sm rounded-xl overflow-hidden border border-black/5 shadow-sm ${!isOnlyAttachment ? "mb-2" : ""}`}
                        >
                          {msg.attachmentType === "image" ||
                          (msg as any).attachment_type === "image" ||
                          (
                            msg.attachmentUrl || (msg as any).attachment_url
                          )?.match(/\.(jpg|jpeg|png|gif|webp)$|images/i) ? (
                            <div
                            ...
                            <a
                              ...
                              className={`flex items-center gap-2 p-2 rounded-lg ${isUser ? "bg-[#1A1A1A]/10" : "bg-black/5"} transition-colors`}
                            >
                      ...
                      {!isOnlyAttachment && (
                        <p
                          className={`text-sm leading-relaxed ${isAdmin ? "font-black" : ""}`}
                        >
                          {msg.content}
                        </p>
                      )}
                ...
                <span
                  className={`text-[9px] font-bold uppercase tracking-tighter opacity-40 px-1 ${isUser ? "text-right" : isAdmin ? "text-center" : "text-left"} ${isAdmin ? "text-red-400" : ""}`}
                >
                  {mounted
                    ? new Date(
