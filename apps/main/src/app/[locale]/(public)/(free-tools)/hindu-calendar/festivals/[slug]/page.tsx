...
export default function FestivalDetailsPage({
  ...
  const [festivalDateItem, setFestivalDateItem] = useState<FestivalItem | null>(
    null,
  );
  const [loading, setLoading] = useState(true);

  // Convert slug back to approximate name (e.g., makar-sankranti -> Makar Sankranti)
  const festivalName = unwrappedParams.slug
    .split("-")
  ...
  const randomFallback =
    fallbackImages[festivalName.length % fallbackImages.length];
    ...
    const fetchDetails = async () => {
      ...
      try {
        ...
        if (festRes.success && festRes.data) {
          ...
          const matched = festRes.data.find(
            (f: any) =>
              f.name.toLowerCase().includes(festivalName.toLowerCase()) ||
              festivalName
                .toLowerCase()
                .includes(f.name.toLowerCase().split("(")[0].trim()),
          );
    ...
    <div className="min-h-screen bg-[#f4f1ea] pt-8 pb-16">
      <div className="max-w-[1320px] mx-auto px-4 md:px-8 lg:px-16">
        ...
        <div className="mb-4 text-sm text-gray-500 flex items-center gap-2 font-sans">
          <Link
            href="/hindu-calendar"
            className="hover:text-[#c85a17] transition-colors font-medium"
          >
        ...
        {loading ? (
          ...
          <div className="bg-white shadow-lg border border-gray-200 overflow-hidden animate-fade-in p-8 md:p-12 relative">
            ...
            <header className="mb-8 border-b-2 border-gray-800 pb-6 text-center">
              ...
              <div className="flex flex-wrap justify-center md:justify-between items-center gap-4 text-sm text-gray-600 font-semibold uppercase tracking-wider font-sans border-t border-b border-gray-300 py-2 px-4">
                <span>
                  ...
                  {festivalDateItem
                    ? format(
                        new Date(festivalDateItem.date),
                        "dd MMMM yyyy, eeee",
                      )
                    : "Date Calculating..."}
                </span>
            ...
            <article
              className="clearfix text-lg text-gray-800 leading-loose"
              style={{ fontFamily: "Georgia, serif" }}
            >
              ...
              <div className="text-gray-700 text-justify">
                {data && data.extract ? (
                  ...
                  <p className="italic text-gray-500">
                    Detailed information about {festivalName} will be updated
                    soon.
                  </p>
                )}
              </div>
              ...
              <section className="bg-[#fffaf5] p-6 md:p-8 rounded-xl border border-[#ff6b00]/20">
                ...
                <div className="mt-6 text-center md:text-left">
                  <Link
                    href="/our-experts"
                    className="inline-block px-6 py-3 bg-[#ff6b00] text-white rounded font-bold shadow hover:bg-[#e65c00] transition-colors font-sans text-sm uppercase tracking-wide"
                  >
