...
export default function SessionSummaryModal({
    ...
    return (
        <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${isDarkMode ? 'bg-[#0a0505]/95' : 'bg-black/60'} backdrop-blur-xl animate-in fade-in duration-500`}>
            <div className={`${isDarkMode ? 'bg-[#1a0c0c] text-white' : 'bg-white text-gray-900'} w-full max-w-md my-auto max-h-[90vh] rounded-[32px] overflow-hidden border ${isDarkMode ? 'border-white/10' : 'border-[#fd6410]/20'} shadow-[0_0_50px_rgba(253,100,16,0.15)] animate-in zoom-in-95 duration-300 relative flex flex-col`}>
                <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    {sessionSummary?.status !== 'terminated' && (
                        <div className={`w-full ${isDarkMode ? 'bg-[#1A1A1A]/5' : 'bg-gray-50'} rounded-2xl p-5 mb-6 space-y-3 flex-shrink-0`}>
                            <div className="flex justify-between items-center text-sm">
                                <span className="opacity-50 font-bold uppercase tracking-tighter">Total Duration</span>
                                <span className="font-black">
                                    {(() => {
                                        const duration = sessionSummary?.durationMins || ((sessionSummary as any)?.duration_seconds ? (sessionSummary as any).duration_seconds / 60 : 0);
                                        return duration ? (
                                            <>
                                                {Math.floor(duration) > 0 && `${Math.floor(duration)} Min `}
                            ...
                            {(sessionSummary?.isFree || (sessionSummary as any)?.is_free) && (
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-xl font-black">₹{sessionSummary?.totalCost ?? (sessionSummary as any)?.total_cost ?? (sessionSummary as any)?.final_price ?? 0}</span>
                            </div>
