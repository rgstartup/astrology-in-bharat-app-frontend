...
export default function ChatFooter({
    ...
    return (
        <footer className={`flex-shrink-0 transition-all duration-300 ${isDarkMode ? 'bg-[#1a0c0c]/80 border-white/5' : 'bg-[#1A1A1A]/80 border-black/5'} backdrop-blur-xl border-t px-3 md:px-12 py-3 md:py-6 shadow-[0_-20px_50px_rgba(0,0,0,0.05)] overflow-visible`}>
            <div className={`relative flex items-center gap-2 md:gap-5 max-w-6xl mx-auto overflow-visible ${sessionStatus !== 'active' ? 'opacity-40 pointer-events-none grayscale' : ''}`}>
                <div className="flex-1 relative group overflow-visible">
                    ...
                    <div className={`relative flex flex-col ${isDarkMode ? 'bg-[#1A1A1A]/5' : 'bg-[#242424]'} border-2 border-[#fd6410] rounded-[20px] md:rounded-[34px] p-1 md:p-1.5 overflow-visible transition-all duration-300 shadow-[0_0_0_4px_rgba(253,100,16,0.1)]`}>
                        {pendingAttachment && (
                        ...
                        <div className="flex items-center gap-1.5 md:gap-4 overflow-visible px-1 md:px-2">
                            <button onClick={() => fileInputRef.current?.click()} className={`shrink-0 p-1.5 md:p-2.5 rounded-full ${isDarkMode ? 'bg-[#1A1A1A]/5 text-gray-400 hover:text-white' : 'bg-[#1A1A1A] text-gray-400 hover:text-[#fd6410]'} transition-all hover:scale-110 active:scale-95 shadow-sm group relative overflow-visible`}>
                            ...
                            <textarea
                                ...
                                className={`flex-1 bg-transparent py-1.5 md:py-2.5 text-sm md:text-base ${isDarkMode ? 'text-white placeholder:text-gray-500' : 'text-gray-200 placeholder:text-gray-400'} outline-none border-none resize-none max-h-28 custom-scrollbar font-medium leading-relaxed m-0`}
                            />
