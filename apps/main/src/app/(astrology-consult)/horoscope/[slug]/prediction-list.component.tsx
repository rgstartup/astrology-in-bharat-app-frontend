...
export default function PredictionList({ horoscope }: { horoscope: any }) {
  ...
  return (
    <div 
      className="rounded-[32px] px-6 pb-8 pt-7 md:px-10 md:pb-10 md:pt-8 border border-[#F0E6DD] shadow-sm relative overflow-hidden bg-white w-full"
      style={{
    ...
    >
      ...
      <p className="text-black text-[15px] md:text-base leading-relaxed mb-8">
        ...
        {horoscope.predictions.map((p: any, idx: number) => {
          ...
          return (
            <div key={idx} className="py-5 md:py-6 flex flex-col gap-3 transition-all">
              ...
              >
                <div className="flex items-center gap-5 flex-1">
                  ...
                  <div className="flex-1">
                    ...
                    {!isExpanded && (
                      <p className="text-black text-[14.5px] md:text-[15px] line-clamp-1 pr-4 leading-relaxed">
                        {p.prediction}
                      </p>
                    )}
                  </div>
              ...
              {/* Expanded Content */}
              {isExpanded && (
                <div className="pt-3 pb-2 pl-[68px] md:pl-[76px] pr-2">
                  <div className="md:hidden mb-4">
                    <StarRating rating={rating} />
                  </div>
                  <p className="text-black text-[15px] md:text-base leading-[1.8] bg-[#FAF8F5] p-5 md:p-6 rounded-2xl border border-[#F0E6DD]">
                    {p.prediction}
                  </p>
                </div>
              )}
