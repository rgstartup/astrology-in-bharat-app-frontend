...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const handleLike = (e: React.MouseEvent) => {
    ...
    toggleLike({ id: String(product.id), type: "product", isLiked });
  };
  ...
  const handleQuickAdd = (e: React.MouseEvent) => {
    ...
    addToCartOpt({ productId: String(product.id), quantity: 1 });
  };
  ...
  return (
    <div
      onClick={() => (onView ? onView(product) : setSelectedProduct(product))}
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-10 flex flex-col items-center gap-1">
        ...
        >
          <i
            className={`${isLiked ? "fa-solid" : "fa-regular"} fa-heart text-base transition-transform active:scale-125`}
          ></i>
        </button>
        {currentLikes > 0 && (
      ...
      <div
        className={`relative w-full ${isCompact ? "aspect-[5/4]" : "aspect-[5/4]"} bg-gray-50/50 flex items-center justify-center overflow-hidden shrink-0 group-hover:bg-white transition-colors duration-500`}
      >
      ...
      {/* 📄 Content Area */}
      <div
        className={`${isCompact ? "p-4 gap-3" : "p-6 gap-4"} flex flex-col flex-grow relative bg-white`}
      >
        <div className={`${isCompact ? "space-y-0.5" : "space-y-1"}`}>
          <div className="flex justify-between items-start gap-2">
            <h3
              className={`${isCompact ? "text-sm" : "text-lg"} font-black text-gray-900 leading-tight group-hover:text-orange transition-colors duration-300`}
              title={product.name}
            >
              {product.name}
            </h3>
          ...
          <p
            className={`${isCompact ? "text-[10px]" : "text-sm"} font-medium text-gray-500 line-clamp-2 leading-snug`}
          >
            {product.description}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 mt-auto">
          <div className="flex items-baseline gap-2">
            <span
              className={`${isCompact ? "text-xl" : "text-2xl"} font-black text-gray-900`}
            >
              ₹{price}
            </span>
            {originalPrice > price && (
