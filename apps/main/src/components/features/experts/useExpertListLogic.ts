  ...
  return {
    ...
    custom_services: Array.isArray(item.custom_services)
      ...
      : typeof item.custom_services === "string"
        ? (() => {
            try {
              return JSON.parse(item.custom_services);
            } catch {
              return [];
            }
          })()
...
export const useExpertListLogic = (
    ...
    if (initialError) {
      toast.error(
        initialError === "server_unreachable"
          ...
          : lang === "hi"
            ? "विशेषज्ञों को लोड करने में विफल"
  ...
  useEffect(() => {
    ...
    const updates = {
      ...
      maxPrice:
        filterState.maxPrice === 1000 ? undefined : filterState.maxPrice,
      state: filterState.addressState,
      service:
        filterState.serviceType === "all" ? undefined : filterState.serviceType,
  ...
  const fetchMoreExperts = useCallback(
    async (currentOffset: number) => {
      try {
        ...
        toast.error(
          lang === "hi"
            ? "और ज्योतिषी लोड करने में विफल"
            : "Failed to load more experts",
        );
      } finally {
        setLoading(false);
      }
