...
export const PujaCard: React.FC<PujaCardProps> = ({ puja }) => {
  ...
  let localizedName = puja.name;
  try {
    localizedName = t.has(`pujaContent.${puja.name}` as any)
      ? t(`pujaContent.${puja.name}` as any)
      : puja.name;
  } catch {
    localizedName = puja.name;
  }

  const localizedDescription =
    puja.description || t("card.descriptionFallback");

  const getMinCost = (pujaItem: ExpertPuja) => {
    const costs = [
      pujaItem.online_cost,
      pujaItem.home_visit_with_samagri_cost,
      pujaItem.home_visit_without_samagri_cost,
    ].filter((cost) => cost > 0);
    ...
    >
      ...
      <div className="px-2 pb-2 flex flex-col grow">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex flex-col">
            <span className="text-[13px] font-medium text-gray-700 mb-0.5">
              {puja.expert?.user?.name || t("card.verifiedPandit")}
            </span>
        ...
        <div className="mt-auto flex items-end justify-between pt-1">
          <div className="flex flex-col">
            <span className="text-[12px] text-gray-700 mb-0.5">
              {t("card.startingFrom")}
            </span>
            <div className="flex items-baseline gap-1">
              <span className="text-[22px] font-bold text-[#FF5500]">
                ₹ {getMinCost(puja).toLocaleString("en-IN")}
              </span>
            </div>
          </div>
