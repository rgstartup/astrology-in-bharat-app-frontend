...
const ExpertFilterModal: React.FC<ExpertFilterModalProps> = ({
  ...
  const { localFilter, setLocalFilter } = useExpertListStore();
        ...
        <div className="p-4 sm:p-6 max-h-[65vh] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <div className="mb-4 sm:mb-6 flex items-center justify-between p-3 sm:p-4 bg-orange/5 rounded-xl sm:rounded-2xl border border-orange/10">
            <div className="flex items-center gap-3">
              <div
                className={`w-3 h-3 rounded-full ${localFilter.onlyOnline ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" : "bg-gray-300"}`}
              ></div>
          ...
          <div className="mb-4 sm:mb-6">
            <label className="block text-sm font-bold text-gray-700 mb-2">
              {t("expertSection.filterLabels.minRating")}
            </label>
            <div className="flex gap-2">
              {[0, 3, 4, 5].map((rating) => (
                <button
                  key={rating}
                  onClick={() =>
                    setLocalFilter({ ...localFilter, minRating: rating })
                  }
                  className={`flex-1 py-2 sm:py-2.5 rounded-xl border text-xs sm:text-sm font-bold transition-all ${
                    localFilter.minRating === rating
                ...
                >
                  {rating === 0 ? (
                    t("expertSection.filterLabels.any")
                  ) : (
          ...
          <div className="mb-4 sm:mb-6">
            ...
            <div className="flex justify-between text-[10px] mt-2 font-medium text-gray-400">
              {[50, 200, 500, 1000].map((p) => (
                <span
                  key={p}
                  className={`cursor-pointer ${localFilter.maxPrice >= p ? "text-orange font-bold" : ""}`}
                >
                  ₹{p}
                </span>
              ))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLocalFilter({ ...localFilter, language: e.target.value })
                }
              />
            </div>
            <div>
