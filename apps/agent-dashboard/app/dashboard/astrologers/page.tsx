...
export default function AstrologersPage() {
    ...
    const stats: StatConfig[] = useMemo(() => [
        { title: "Total", value: data.length, icon: Star, iconColor: "text-yellow-600", iconBgColor: "bg-yellow-100" },
        ...
        { title: "Pending", value: data.filter((d) => d.status === "pending").length, icon: Clock, iconColor: "text-orange-600", iconBgColor: "bg-orange-100" },
                    ...
                    {data.map((a) => {
                        ...
                        return (
                            <div key={a.id || a._id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-yellow-400 transition-all p-5">
                                ...
                                <h4 className="font-black text-gray-900 text-sm">{a.name}</h4>
                                <p className="text-xs text-primary-hover font-semibold mt-0.5">{a.specialization || "—"}</p>
                                <div className="mt-3 space-y-1.5">
                                    {a.location && <p className="text-xs text-gray-500 flex items-center gap-1.5"><MapPin className="w-3 h-3" />{a.location}</p>}
                                    {a.phone && <p className="text-xs text-gray-500 flex items-center gap-1.5"><Phone className="w-3 h-3" />{a.phone}</p>}
                                </div>
                                <p className="text-[10px] text-gray-400 mt-3 font-medium">Listed on {a.createdAt ? new Date(a.createdAt).toLocaleDateString() : '—'}</p>
                            </div>
                        );
                    })}
                </div>
