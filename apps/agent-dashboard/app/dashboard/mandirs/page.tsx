...
const EMPTY_FORM = { name: "", deity: "", location: "", phone: "" };
...
export default function MandirstPage() {
    ...
    const fetchListings = useCallback(async () => {
        try {
            ...
            toast.error("Failed to load mandir listings");
        } finally {
            setLoading(false);
        }
    }, [search]);
    ...
    const stats: StatConfig[] = useMemo(() => [
        ...
        { title: "Approved", value: data.filter((d) => d.status === "approved").length, icon: CheckCircle, iconColor: "text-green-600", iconBgColor: "bg-green-100" },
        { title: "Pending", value: data.filter((d) => d.status === "pending").length, icon: Clock, iconColor: "text-yellow-600", iconBgColor: "bg-yellow-100" },
        { title: "Rejected", value: data.filter((d) => d.status === "rejected").length, icon: XCircle, iconColor: "text-red-500", iconBgColor: "bg-red-50" },
    ], [data]);
            ...
            {showForm && (
                <div className="bg-orange-50 border border-orange-200 rounded-2xl p-6">
                    ...
                    <form onSubmit={handleAdd} className="grid sm:grid-cols-2 gap-4">
                        {[
                            { key: "name", placeholder: "Mandir name *", label: "Mandir Name *", required: true },
                            ...
                            <div key={key}>
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-2.5 rounded-xl border border-orange-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
                                />
                            </div>
                        ))}
            ...
            {loading ? (
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {data.map((m) => {
                        ...
                        const Icon = St.icon;
                        return (
                            <div key={m.id || m._id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-orange-300 transition-all p-5">
