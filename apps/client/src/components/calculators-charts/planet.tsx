...
} from "@/app/[locale]/(public)/astrology/calculator/planet-calculator/calculate";

const Planet = () => {
  const { lang, toggleLang } = useLanguageStore();
  const t =
    planetTranslations[lang as keyof typeof planetTranslations] ||
    planetTranslations.en;
          ...
          <div className="w-full lg:w-2/3">
            ...
            {loading && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div
                    key={i}
                    className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 animate-pulse"
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {planetData.map((planet: any, index: number) => {
                ...
                const signName =
                  t.zodiacs[signIndex - 1] || t.planets["Unknown"];
                const translatedPlanetName =
                  (t.planets as any)[planet.name] || planet.name;

                // Defensive value extraction
                const normDegree =
                  typeof planet.normDegree === "number" ? planet.normDegree : 0;
                const fullDegree =
                  typeof planet.fullDegree === "number" ? planet.fullDegree : 0;
                      ...
                      <div className="bg-slate-50 rounded-2xl p-4 space-y-3">
                        <div className="flex justify-between items-center text-sm">
                          ...
                          <span className="text-slate-700 font-bold">
                            {normDegree.toFixed(2)}°
                          </span>
                        </div>
                        <div className="h-px bg-slate-100 w-full"></div>
                        <div className="flex justify-between items-center text-sm">
                          <span className="text-slate-400 font-medium tracking-wide">
                            {t.results.fullDegree}
                          </span>
