...
const DUMMY_PUJAS: any[] = [
  {
    ...
    description:
      "Hanuman Chalisa Path for peace, prosperity and protection from evil eyes.",
  ...
  },
  {
    ...
    name: "Saraswati Puja",
    description:
      "Seek blessings of Goddess Saraswati for knowledge and wisdom.",
    puja_mode: "Offline",
...
const PujaListSection = () => {
  ...
  useEffect(() => {
    const fetchPujasItems = async () => {
      setLoading(true);
      const [res, error] = (await http.get<ExpertPuja[]>(
  ...
  const filteredPujas = pujas.filter((puja) => {
    ...
    const matchesDropdown =
      selectedPujaName === t.filters.allPujas || puja.name === selectedPujaName;
    return matchesSearch && matchesDropdown;
  });

  const displayPujas =
    !loading && filteredPujas.length > 0 && filteredPujas.length < 3
      ? [...filteredPujas, ...DUMMY_PUJAS.slice(0, 3 - filteredPujas.length)]
      : !loading &&
          filteredPujas.length === 0 &&
          searchQuery === "" &&
          selectedPujaName === t.filters.allPujas
        ? DUMMY_PUJAS
        : filteredPujas;
        ...
        <div className="relative mb-4 md:mb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-2 md:gap-4 z-20">
          ...
          >
            <h2 className="section-heading-premium mb-0">
              <span>
                {t.page.title}{" "}
                <span className="text-orange-600">{t.page.titleHighlight}</span>
              </span>
            </h2>
          </div>
            ...
            <div className="relative w-full sm:w-48 md:w-56" ref={dropdownRef}>
              ...
              >
                <span className="truncate pr-2">
                  {selectedPujaName === t.filters.allPujas
                    ? t.filters.allPujas
                    : content[selectedPujaName] || selectedPujaName}
                </span>
                <ChevronDown
