...
export default function FestivalCarouselWidget({
  lang,
  festivals = [],
}: Props) {
          ...
          {festivals.length === 0
            ? // Skeleton Loading State
              Array.from({ length: 6 }).map((_, idx) => (
                <div
                  key={`skel-${idx}`}
                  className="min-w-[160px] md:min-w-[180px] flex-shrink-0 bg-[#fffaf5] rounded-xl border-2 border-orange-100 p-4 shadow-sm flex flex-col items-center justify-center animate-pulse"
                >
            ...
            : displayFestivals.map((fest, idx) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`text-4xl mb-3 drop-shadow-sm ${style.color}`}
                    >
                      <i className={`fa-solid ${style.icon}`}></i>
                    </div>
                    <h4 className="text-[#5b2a26] font-bold text-sm mb-1 leading-tight h-10 flex items-center justify-center">
                      {fest.name}
                    </h4>
