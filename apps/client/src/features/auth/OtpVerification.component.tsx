...
export const OtpVerification: React.FC<OtpVerificationProps> = ({
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast.error(err?.message || t("signUp.errors.unexpected"));
    } finally {
  ...
  return (
    <div className="w-full max-w-[460px] sm:max-w-[480px] mx-auto lg:mx-0 py-0">
      {/* Top Header */}
      <AuthHeader subtitle={subtitle || t("signUp.header")} />
          ...
          {countdown > 0 ? (
            ...
            <p className="flex items-center justify-center gap-1">
              ...
              <button
                ...
                className="text-orange font-bold hover:underline cursor-pointer disabled:opacity-50 inline-flex items-center gap-1.5"
              >
                {isResending ? (
                  <>
                    <Loader2 className="size-3 animate-spin text-orange" />
                    <span>{t("signUp.resendingOtp") || "Resending..."}</span>
                  </>
                ) : (
                  <span>{t("signUp.resendOtp")}</span>
                )}
              </button>
            </p>
          )}
