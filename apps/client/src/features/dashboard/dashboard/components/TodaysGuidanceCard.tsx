...
export function TodaysGuidanceCard({
  ...
  const getScoreColor = (score: number) => {
    ...
    if (score >= 65) return "text-slate-700 bg-slate-50 border-slate-200";
    return "text-orange-700 bg-slate-50 border-slate-200";
  };

  const getScoreProgressBg = (score: number) => {
    if (score >= 80) return "bg-emerald-500";
    if (score >= 65) return "bg-slate-400";
    return "bg-orange-500";
  };

  if (isLoading) {
    return (
      <Card className="mb-6 sm:mb-8 border-slate-200 bg-white p-6 space-y-4">
  ...
  return (
    <Card className="mb-6 sm:mb-8 border-slate-200 bg-white shadow-sm overflow-hidden">
      {/* Header bar */}
