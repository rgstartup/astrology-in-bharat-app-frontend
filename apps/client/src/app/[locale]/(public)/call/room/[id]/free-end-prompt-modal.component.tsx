...
export default function FreeEndPromptModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
      <div
        className={`${isDarkMode ? "bg-[#1a0c0c] border-white/10" : "bg-neutral-900 border-white/10"} w-full max-w-sm rounded-[32px] overflow-hidden border p-8 text-center shadow-2xl relative text-white`}
      >
        {/* 30s Countdown Ring */}
        <div className="absolute top-4 right-4 w-10 h-10 rounded-full border-2 border-[#fd6410]/20 flex items-center justify-center">
          <span className="text-[10px] font-black text-[#fd6410]">
            {continuationTimer}s
          </span>
        </div>
        ...
        <p className="text-xs opacity-60 leading-relaxed mb-6">
          {freeLimitData?.message ||
            `Your free consultation is over. Continue at ₹${freeLimitData?.expertPrice || expertData?.price}/min?`}
        </p>
        ...
        <div className="flex flex-col gap-3">
          {freeLimitData?.requireRecharge ? (
            ...
            <button
              onClick={() => {
                socket?.emit("confirm_paid_continuation", {
