...
export default function FreeEndPromptModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
      <div
        className={`${isDarkMode ? "bg-[#1a0c0c] border-white/10" : "bg-[#1A1A1A] border-[#fd6410]/20"} w-full max-w-sm rounded-[32px] overflow-hidden border p-8 text-center shadow-2xl relative`}
      >
        {/* 30s Countdown Ring */}
        ...
        <div className="flex flex-col gap-3">
          {freeLimitData?.requireRecharge ? (
            ...
            <button
              onClick={() => {
                chatSocket.emit("confirm_paid_continuation", {
                  sessionId: sessionId,
                });
              }}
