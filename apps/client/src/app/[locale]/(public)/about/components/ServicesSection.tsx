          ...
          {services.map((svc, i) => {
            ...
            const cardContent = (
              <div
                className={`group relative bg-gradient-to-br from-[#2b1117] to-[#3e1b23] rounded-[2.5rem] p-10 flex flex-col items-center text-center overflow-hidden transition-all duration-500 h-full border border-white/5 shadow-2xl ${
                  isClickable
                    ? "hover:-translate-y-3 hover:shadow-orange-500/20 active:scale-95"
                    : ""
                }`}
              >
            ...
            return isClickable ? (
              <Link
                key={i}
                href={svc.link!}
                className="block h-full no-underline animate-in fade-in slide-in-from-bottom-8 duration-700"
