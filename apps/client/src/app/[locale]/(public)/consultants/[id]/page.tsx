...
import { getErrorMessage, PaginatedProductsResponse } from "@repo/lib";
...
async function AsyncPujas({
  ...
  try {
    const pujaData = await api.get<any>("/expert/pujas/all", {
      cache: "no-store",
    });
        ...
        <div className="max-w-[1320px] mx-auto px-4 md:px-8 lg:px-16">
          <div className="mb-8">
            ...
            <p className="text-gray-500 font-medium">
              Book a personalized puja with this expert astrologer.
            </p>
          </div>
...
async function AsyncProducts() {
  try {
    const [data, error] = await api.get<PaginatedProductsResponse>(
      `/products`,
      {
        cache: "no-cache",
      },
...
export default async function Page({
  ...
  try {
    ...
    if (id.startsWith("dummy-")) {
      ...
      const result = await api.get<any>(`/expert/account/${id}`, {
        cache: "no-store",
      });
      data = result[0];
      astroError = result[1];
    ...
    if (astroError) {
      if (astroError.status === 404 || astroError.status === 400)
        return notFound();
      throw new Error(
        getErrorMessage(astroError) || "Failed to fetch expert details",
      );
    }
        ...
        <Suspense
          fallback={
            <div className="py-20 flex justify-center">
              <Loading size="md" text="Loading Pujas..." />
            </div>
          }
        >
          {id.startsWith("dummy-") ? null : (
            <AsyncPujas expertId={expert.id} expertName={expert.name} />
          )}
        </Suspense>
    ...
    return (
      <div className="container py-20 text-center">
        ...
        <p>We couldn&apos;t load the expert details. Please try again later.</p>
      </div>
    );
  }
}
