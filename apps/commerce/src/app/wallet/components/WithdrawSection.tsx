        ...
        <div className="group bg-white p-10 rounded-[1.5rem] border-2 border-orange-600 shadow-sm space-y-8 relative transition-all duration-700 hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.05)] hover:-translate-y-1">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row items-end gap-6 relative z-10">
                <div className="flex-1 w-full space-y-2.5 relative">
                    <label className="text-sm font-semibold text-gray-700 ml-1">Amount to Withdraw</label>
                    ...
                    <p className="absolute -bottom-5 text-[11px] text-gray-700 font-medium tracking-tight ml-1 flex items-center gap-1">
                        <Info className="w-3.5 h-3.5 text-gray-500" /> Min. withdrawal ₹500
                    </p>
                </div>

                <div className="flex-1 w-full space-y-2.5 relative">
                    <label className="text-sm font-semibold text-gray-700 ml-1">Select Bank Account</label>
                    {!hasBankDetails ? (
                        <div className="flex items-center gap-3 px-5 h-16 bg-red-50 border border-red-100 rounded-2xl text-red-500">
                            <AlertCircle className="w-4 h-4 shrink-0" />
                            <p className="text-sm font-semibold tracking-tight italic">Please add a bank account first</p>
                        </div>
                    ) : (
                        <div className="relative">
                            <button
                            ...
                            {showBankList && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 rounded-2xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                                    <div className="p-4 bg-gray-50/50 border-b border-gray-100">
                                        <p className="text-sm font-semibold text-gray-500">Select Account</p>
                                    </div>
                ...
                <div className="flex-1 w-full space-y-2.5">
                    <label className="text-sm font-semibold opacity-0 ml-1 block select-none pointer-events-none">Action</label>
                    <div className="w-full">
                        <Button
                            type="submit"
                            disabled={loading || !amount || parseFloat(amount) < 500 || parseFloat(amount) > balance || !hasBankDetails}
                            className="w-full h-16 rounded-[1.5rem] bg-[#fd6410] hover:bg-orange-600 text-white font-bold text-sm px-10 transition-all duration-300 shadow-lg shadow-orange-500/20 disabled:opacity-50 disabled:grayscale active:scale-95 border-none"
                        >
